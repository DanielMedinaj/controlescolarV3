...
if(!$_POST){
    ...
    if (isset($_POST['insertar'])){
        ...
        if (isset($result)) {
            header('location:notas.view.php?materia='.$id_materia.'&revisar=1&info=1');
        } else {
            header('location:notas.view.php?materia='.$id_materia.'&revisar=1&err=1');
    ...
    //sino boton modificar que esta en el archivo alumnoedit.view.php
        ...
        //capturamos el id alumnos a modificar
            ...
            $result = $conn->query("update alumnos set num_lista = '$numlista', nombres = '$nombres', apellidos = '$apellidos', genero = '$genero' where id = " . $id_alumno);
            if (isset($result)) {
